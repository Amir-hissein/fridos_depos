...
export default function CalorieRecipesScreen() {
  ...
  const sections = useMemo(() => {
    ...
    const snack = filteredRecipes.filter(
      (r) =>
        r.mealType === 'Ara Öğün' ||
        r.categories?.includes('fit-desserts') ||
        r.categories?.includes('fruits'),
    );
    const main = filteredRecipes.filter(
      (r) =>
        ['Ana Öğün', 'Öğle Yemeği', 'Akşam Yemeği', 'Ana Yemek'].includes(r.mealType) ||
        r.categories?.includes('light-dinner'),
    );
      ...
      <SafeAreaView style={styles.safe} edges={['top']}>
        ...
        >
          ...
          {filteredRecipes.length === 0 ? (
            <FadeInItem style={styles.emptyContainer}>
            ...
            <>
              ...
              {featuredRecipe && (
                <FadeInItem index={0} style={styles.section}>
                  ...
                  >
                      ...
                      <View style={styles.featuredBadges}>
                        ...
                        <View style={styles.featuredBadge}>
                          ...
                          <Text style={styles.featuredBadgeText}>
                            {featuredRecipe.time} {t('plan.min')}
                          </Text>
                        </View>
                      </View>
                    </View>
                    <View style={styles.featuredInfo}>
                      <Text style={styles.featuredTitle}>
                        {localizeRecipeName(featuredRecipe, t)}
                      </Text>
              ...
              {sections.map((sect, sIdx) => (
                <FadeInItem key={sIdx} index={1 + sIdx} style={styles.section}>
                  ...
                  <FlatList
                    ...
                    renderItem={({ item }) => (
                      <RecipeCardB recipe={item} onPress={() => openRecipe(item.id)} />
                    )}
                  />
                </FadeInItem>
              ))}
...
const makeStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    ...
    featuredImgArea: {
      ...
      alignItems: 'center',
      justifyContent: 'center',
      // Clip the cover image here so the card can cast an (unclipped) shadow.
      overflow: 'hidden',
