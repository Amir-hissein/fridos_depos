...
const ACTIVITY_OPTIONS = [
  {
    key: 'sedentary',
    labelKey: 'profile.personalInfo.activityOptions.sedentary.label',
    descKey: 'profile.personalInfo.activityOptions.sedentary.desc',
  },
  {
    key: 'active',
    labelKey: 'profile.personalInfo.activityOptions.active.label',
    descKey: 'profile.personalInfo.activityOptions.active.desc',
  },
  {
    key: 'cardio',
    labelKey: 'profile.personalInfo.activityOptions.cardio.label',
    descKey: 'profile.personalInfo.activityOptions.cardio.desc',
  },
];
...
export default function PersonalInfoScreen() {
  ...
  const activityLabel = tempActivity
    ? t(`profile.personalInfo.activityOptions.${tempActivity}.label`)
    : '';
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      >
        ...
        <FadeInItem index={0} style={s.group}>
          <InfoRow
            ...
            value={
              tempSex === 'male' ? t('profile.personalInfo.male') : t('profile.personalInfo.female')
            }
            ...
            {[
              {
                ...
                label: t('profile.personalInfo.targets.calorie'),
                value: `${targets.kcal}`,
                unit: 'kcal',
                color: colors.orange,
              },
              {
                icon: 'bread-slice' as IconName,
                label: t('profile.personalInfo.targets.carbs'),
                value: `${targets.carbs}`,
                unit: 'g',
                color: colors.gold,
              },
              {
                ...
                label: t('profile.personalInfo.targets.protein'),
              ...
              },
              {
                ...
                label: t('profile.personalInfo.targets.fat'),
                value: `${targets.fat}`,
                unit: 'g',
                color: colors.purple,
              },
      ...
      {/* ── Sex Modal ── */}
      <EditModal
        visible={editModal === 'sex'}
        title={t('profile.personalInfo.gender')}
        onClose={closeModal}
      >
      ...
      {/* ── Age Modal ── */}
      <EditModal
        visible={editModal === 'age'}
        title={`${t('profile.personalInfo.age')}: ${t('profile.personalInfo.ageValue', { count: tempAge })}`}
        onClose={closeModal}
      >
        <Slider
      ...
      <EditModal
        ...
        title={`${t('profile.personalInfo.height')}: ${t('profile.personalInfo.heightValue', { count: tempHeight })}`}
        onClose={closeModal}
      >
        <Slider
          style={{ marginHorizontal: 8, marginVertical: 20 }}
        ...
        <Text style={m.sliderVal}>
          {t('profile.personalInfo.heightValue', { count: tempHeight })}
        </Text>
        <PressableScale
          haptic="light"
          style={m.saveBtn}
          onPress={() => {
            updateProfile({ height: tempHeight });
            haptic.success();
            closeModal();
          }}
          activeOpacity={0.8}
      ...
      <EditModal
        ...
        title={`${t('profile.personalInfo.weight')}: ${t('profile.personalInfo.weightValue', { count: tempWeight })}`}
        onClose={closeModal}
      >
        <Slider
          style={{ marginHorizontal: 8, marginVertical: 20 }}
      ...
      <EditModal
        ...
        title={`${t('profile.personalInfo.targetWeight')}: ${t('profile.personalInfo.weightValue', { count: tempTargetWeight })}`}
        onClose={closeModal}
      >
        <Slider
          style={{ marginHorizontal: 8, marginVertical: 20 }}
      ...
      {/* ── Goal Modal ── */}
      <EditModal
        visible={editModal === 'goal'}
        title={t('profile.personalInfo.goal')}
        onClose={closeModal}
      >
        {GOAL_OPTIONS.map((g) => (
          <PressableScale
            ...
            activeOpacity={0.8}
          >
            <Text style={[m.optionText, tempGoal === g.key && m.optionTextActive]}>
              {t(g.labelKey)}
            </Text>
            {tempGoal === g.key && (
              <MaterialCommunityIcons name="check-circle" size={20} color={colors.green} />
      ...
      {/* ── Activity Modal ── */}
      <EditModal
        visible={editModal === 'activity'}
        title={t('profile.personalInfo.activity')}
        onClose={closeModal}
      >
        {ACTIVITY_OPTIONS.map((a) => (
          <PressableScale
            ...
            onPress={() => {
              haptic.select();
              setTempActivity(a.key as any);
            }}
            activeOpacity={0.8}
          >
            <View style={{ flex: 1 }}>
              <Text style={[m.optionText, tempActivity === a.key && m.optionTextActive]}>
                {t(a.labelKey)}
              </Text>
        ...
        <PressableScale
          ...
          style={m.saveBtn}
          onPress={() => {
            updateProfile({ activity: tempActivity });
            haptic.success();
            closeModal();
          }}
          activeOpacity={0.8}
