    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      >
        ...
        {activeTab === 'fridge' && (
          <>
            {/* Stats */}
            <FadeInItem index={0} style={s.statsRow}>
            ...
            {Object.keys(grouped).length > 0 ? (
              Object.entries(grouped).map(([catKey, items], gi) => {
                const ci = getCategoryForItem(items[0]);
                return (
                  <FadeInItem key={catKey} index={2 + gi} style={s.categorySection}>
              ...
              <FadeInItem index={2} style={s.emptyState}>
          ...
          <>
            {recentItems.length > 0 ? (
              recentItems.map((item, idx) => {
                ...
                return (
                  <FadeInItem key={`${item}-${idx}`} index={idx} style={s.recentItem}>
                    <View style={s.recentDot} />
                    <View style={{ flex: 1 }}>
                      <Text style={s.recentName}>{item}</Text>
                      <Text style={s.recentCat}>
                        {t(`profile.fridgePage.categories.${ci.key}`)}
                      </Text>
                    </View>
                    <PressableScale
                      haptic="light"
        ...
        {activeTab === 'history' && (
          <>
            {pastRecipes.length > 0 ? (
              pastRecipes.map((recipe, ri) => (
                <FadeInItem key={recipe.id} index={ri}>
                  <PressableScale
                    haptic="light"
                    style={s.recipeCard}
                    activeOpacity={0.8}
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={s.recipeDate}>
                        {recipe.time} {t('plan.min')} · {recipe.kcal} kcal
                      </Text>
                    </View>
                    <MaterialCommunityIcons
                      name="chevron-right"
