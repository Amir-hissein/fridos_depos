  ...
  return (
    <View
      style={[
        style,
        {
          backgroundColor: bgColor ?? colors.backgroundAlt,
          alignItems: 'center',
...
export default function MealDetailScreen() {
  ...
  const { intake, mealTargets, mealMacrosFor, addCustomKcal, toggleMealRecipe, isRecipeLogged } =
    usePlan();
  ...
  const recommended = useMemo(() => {
    const byMeal = filterRecipes(RECIPES, {
      meal: mealSlot as MealType,
      excludeAllergens: userAllergens,
    });
    const pool =
      byMeal.length >= 3 ? byMeal : filterRecipes(RECIPES, { excludeAllergens: userAllergens });
    return [...pool]
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        ...
        <FadeInItem index={1} style={styles.infoBox}>
          <View style={styles.infoIconBox}>
            <Ionicons name="sparkles" size={18} color={colors.white} />
          </View>
          <Text style={styles.infoText}>{t('mealDetail.infoText')}</Text>
        </FadeInItem>

        {/* ══ ÖNERİLEN TARİFLER ══════════════════════════════ */}
        <FadeInItem index={2} style={styles.sectionHeader}>
          ...
          >
            <Text style={styles.hideText}>
              {showRecipes ? t('mealDetail.hideRecipes') : t('mealDetail.showRecipes')}
            </Text>
          </PressableScale>
        </FadeInItem>
            ...
            <View style={styles.quickGrid}>
              {[
                ...
                const localizedName = t(
                  `mealDetail.addFoodModal.quickItems.${item.key}`,
                  item.fallback,
                );
                return (
                  <PressableScale
...
const makeStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    ...
    recipeImg: {
      width: '100%',
      height: 140,
      // Round the top image corners here so the card shadow isn't clipped.
