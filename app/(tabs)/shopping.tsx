...
const getCategoryLabel = (catLabel: string, t: any) => {
  const clean = catLabel
    .replace(/[🥬🧀🛒📦]/g, '')
    .trim()
    .toLowerCase();
  if (clean.includes('sebze') || clean.includes('fruit') || clean.includes('produce'))
    return t('shopping.categories.fruitsVeg');
  if (clean.includes('süt') || clean.includes('dairy')) return t('shopping.categories.dairy');
  if (
    clean.includes('temel') ||
    clean.includes('pantry') ||
    clean.includes('kiler') ||
    clean.includes('gıda')
  )
    return t('shopping.categories.pantry');
...
const CATEGORIES = [
  {
    ...
    label: 'Produce',
    icon: 'fruit-watermelon' as const,
    color: Colors.green,
  },
            ...
            {doneItems > 0 && (
              <PressableScale
                ...
                style={[
                  ...
                  { backgroundColor: colors.orangeLight, marginRight: 8 },
                ]}
      ...
      <View style={styles.tabRow}>
        ...
        >
          <Text style={[styles.tabText, activeTab === 'tobuy' && styles.tabTextActive]}>
            {t('shopping.tabs.toBuy')}
            {derivedMissing.length > 0 ? ` (${derivedMissing.length})` : ''}
          </Text>
        ...
        {activeTab === 'tobuy' ? (
          derivedMissing.length > 0 ? (
                    ...
                    {catItems.map((m, ii) => (
                      <React.Fragment key={`${m.name}-${ii}`}>
                        ...
                        <View style={styles.buyRow}>
                          ...
                          <PressableScale
                            haptic="light"
                            style={styles.buyAddBtn}
                            onPress={() => addMissingToList(m.name, m.category)}
                          >
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySub}>{t('shopping.emptyTobuySub')}</Text>
            </View>
          )
