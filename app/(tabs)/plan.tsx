...
const MEALS: MealSlotDef[] = [
  {
    key: 'breakfast',
    label: 'plan.meals.breakfast',
    image: require('../../assets/images/breakfast_icon.png'),
    color: Colors.gold,
  },
  {
    key: 'lunch',
    label: 'plan.meals.lunch',
    image: require('../../assets/images/lunch_icon.png'),
    color: Colors.green,
  },
  {
    key: 'dinner',
    label: 'plan.meals.dinner',
    image: require('../../assets/images/dinner_icon.png'),
    color: Colors.blue,
  },
  {
    key: 'snack',
    label: 'plan.meals.snack',
    image: require('../../assets/images/snack_icon.png'),
    color: Colors.orange,
  },
];
    ...
    >
      ...
      <View style={mc.topRow}>
        <View style={[mc.iconBox, { backgroundColor: mealColor + '22' }]}>
          <Image
            source={slot.image}
            style={{ width: 28, height: 28, tintColor: mealColor }}
            resizeMode="contain"
          />
        </View>
        <Text style={mc.label} numberOfLines={2}>
          {t(slot.label)}
        </Text>
      </View>
...
const makeMcStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    card: {
      ...elevation(colors, 1),
...
function Tracker({
  ...
  icon: MCIName;
  title: string;
  subParts: { text: string; bold?: boolean }[];
  last?: boolean;
  onPress?: () => void;
}) {
  const { colors } = useTheme();
...
const makeTrStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    row: {
      flexDirection: 'row',
      alignItems: 'center',
...
export default function PlanScreen() {
  ...
  const trackerGoal =
    activeTracker === 'water' ? waterGoal : activeTracker === 'steps' ? stepsGoal : 0;
  const trackerCurrent =
    activeTracker === 'water' ? tempWater : activeTracker === 'steps' ? tempSteps : tempWeight;
  const trackerUnit =
    activeTracker === 'water' ? 'ml' : activeTracker === 'steps' ? t('plan.trackers.step') : 'kg';
  ...
  const isCurrentMonth =
    currentYearMonth.month === today.getMonth() && currentYearMonth.year === today.getFullYear();
  ...
  const recipes = recommendRecipes(RECIPES, {
    diet: profile.diet,
          ...
          <View style={styles.sheetContent}>
            ...
            <View style={styles.sheetHeader}>
              <View style={styles.sheetHeaderLeft}>
                ...
                <Text style={styles.sheetTitle}>
                  {activeTracker === 'water'
                    ? t('plan.trackers.water')
                    : activeTracker === 'steps'
                      ? t('plan.trackers.steps')
                      : t('plan.trackers.weight')}
                </Text>
              </View>
              <PressableScale
                haptic="light"
            ...
            <View style={styles.controlRow}>
              ...
              <View style={styles.controlValueWrap}>
                ...
                {trackerGoal > 0 && (
                  <Text style={styles.controlGoal}>
                    / {trackerGoal} {trackerUnit}
                  </Text>
                )}
              </View>
            ...
            {trackerProgress ? (
              <View style={styles.sheetProgressWrap}>
                ...
                <Text style={styles.sheetProgressLabel}>
                  {t('plan.trackers.remainingGoal', {
                    percent: trackerProgress.percent,
                    remaining: trackerProgress.remaining,
                    unit: trackerUnit,
                  })}
                </Text>
              </View>
              ...
              <Text style={styles.weightGoalText}>
                {t('plan.trackers.weightToGo', {
                  kg: Math.max(0, tempWeight - profile.targetWeight).toFixed(1),
                })}
              </Text>
            )}
      ...
      >
        ...
        <View style={{ zIndex: 100 }}>
          <View style={[styles.header, { zIndex: 105 }]}>
            <Text style={[styles.greeting, calendarOpen && { opacity: 0 }]}>
              {t('plan.greeting', { name: userName ? userName.split(' ')[0] : '' })}
            </Text>

            <PressableScale
              style={[
                styles.datePill,
                calendarOpen && { backgroundColor: colors.surface, borderColor: colors.gold },
              ]}
              scaleTo={0.95}
              haptic="light"
              onPress={() => {
            ...
            >
              <Ionicons
                ...
                size={16}
                color={calendarOpen ? colors.gold : colors.textPrimary}
              />
              <Text style={[styles.dateStr, calendarOpen && { color: colors.gold }]}>
                {activeDateStr}
              </Text>
              <Ionicons
                name={calendarOpen ? 'chevron-up' : 'chevron-down'}
                size={14}
                color={calendarOpen ? colors.gold : colors.textMuted}
              />
            </PressableScale>
          </View>
          ...
          {calendarOpen && (
            <>
              ...
              />
              <View
                style={[styles.calendarDropdownCard, { top: 50, left: 0, right: 0, zIndex: 110 }]}
              >
                {/* Nav Row */}
                <View style={styles.calNavRow}>
                  ...
                  <View style={styles.calHeaderTitleWrap}>
                    <Text style={styles.calMonthName}>{months[currentYearMonth.month]}</Text>
                    <Text style={styles.calYearName}>{currentYearMonth.year}</Text>
                  </View>
                ...
                <View style={styles.calDaysHeaderRow}>
                  {weekdays.map((day, i) => (
                    <Text
                      key={day}
                      style={[
                        styles.calDayHeaderTxt,
                        (i === 5 || i === 6) && styles.calDayHeaderTxtWeekend,
                      ]}
                    >
                      {day}
                    </Text>
                  ...
                  <View style={styles.macroProgressBarBg}>
                    <View
                      style={[
                        ...
                        {
                          width:
                            `${Math.min((consumedCarbs / targets.carbs) * 100, 100)}%` as `${number}%`,
                        },
                      ]}
                    />
                  </View>
                  ...
                  <View style={styles.macroProgressBarBg}>
                    <View
                      style={[
                        ...
                        {
                          width:
                            `${Math.min((consumedProtein / targets.protein) * 100, 100)}%` as `${number}%`,
                        },
                      ]}
                    />
                  </View>
                  ...
                  <View style={styles.macroProgressBarBg}>
                    <View
                      style={[
                        ...
                        {
                          width:
                            `${Math.min((consumedFat / targets.fat) * 100, 100)}%` as `${number}%`,
                        },
                      ]}
                    />
                  </View>
        ...
        <FadeInItem index={3}>
          <View style={styles.trackCard}>
            ...
            <Tracker
              ...
              subParts={[
                { text: `${intake.steps || 0} ${t('plan.trackers.step')}`, bold: true },
                { text: ` / ${stepsGoal} ${t('plan.trackers.step')}.` },
              ]}
              onPress={() => {
                haptic.light();
            ...
            <Tracker
              ...
              subParts={[
                {
                  text: t('plan.trackers.weightToGo', {
                    kg: Math.max(0, profile.weight - profile.targetWeight).toFixed(1),
                  }),
                  bold: true,
                },
              ]}
          ...
          >
            {recipes.map((r) => (
              ...
              >
                ...
                {r.image ? (
                  <Image
                    source={recipeImageSource(r.image)}
                ...
                <View style={styles.recipeInfo}>
                  ...
                  <View style={styles.recipeMeta}>
                    ...
                    <Text style={styles.recipeMetaTxt}>
                      {r.time} {t('plan.min')}
                    </Text>
                  </View>
                </View>
...
const makeStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    ...
    recipeImg: {
      width: '100%',
      height: 130,
      // Round top corners on the image so the card shadow isn't clipped.
