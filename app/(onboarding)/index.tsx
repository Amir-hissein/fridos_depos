...
/* ─── Detection Badge (fades + slides in as the AI "locks on" to an item) ─── */
function DetectBadge({
  ...
  const style = useAnimatedStyle(() => {
    const p = interpolate(progress.value, [appearAt, appearAt + 0.12], [0, 1], Extrapolation.CLAMP);
    return {
      opacity: p,
      transform: [{ translateX: (1 - p) * 26 }, { scale: 0.88 + p * 0.12 }],
    };
  });
  return (
    <Animated.View
      style={[styles.statusBadge, styles.detectBadge, { borderColor: color + '99' }, style]}
    >
      <Ionicons name="checkmark-circle" size={13} color={color} style={{ marginRight: 6 }} />
...
export default function CinematicOnboarding() {
  ...
  // Viewfinder "lock-on": brackets snap into place at the very start.
  const bracketsStyle = useAnimatedStyle(() => {
    const p = interpolate(progress.value, [0, 0.18], [0, 1], Extrapolation.CLAMP);
    return { opacity: p, transform: [{ scale: 1.14 - p * 0.14 }] };
  });

  // Scan beam sweeps up and down forever — the scanner never stops.
  const laserStyle = useAnimatedStyle(() => {
    const translateY = interpolate(laserPos.value, [0, 1], [-90, 90]);
    return { transform: [{ translateY }] };
  });

  // Pulsing heartbeat on the "scanning" dot.
  const dotStyle = useAnimatedStyle(() => ({
    opacity: interpolate(dotPulse.value, [0, 1], [0.35, 1]),
    transform: [{ scale: interpolate(dotPulse.value, [0, 1], [0.8, 1.25]) }],
  }));
    ...
    <View style={styles.container}>
      <View style={StyleSheet.absoluteFill}>
        ...
        <Animated.View style={StyleSheet.absoluteFill}>
          <Image
            source={require('../../assets/images/onboard_scanner.png')}
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
        ...
        <View style={[styles.contentLayer, { pointerEvents: 'none' }]}>
          ...
          <View style={styles.viewfinder}>
            <Animated.View style={[StyleSheet.absoluteFill, bracketsStyle]}>
            ...
            {/* Scan beam with a soft trailing glow */}
            <Animated.View style={[styles.laserWrap, laserStyle]}>
              <LinearGradient
                colors={[colors.green + '00', colors.green + '3D']}
                start={{ x: 0, y: 0 }}
                end={{ x: 0, y: 1 }}
          ...
          {/* Right HUD Scan Status — perpetual "scanning" pill + detected items */}
