...
export default function FridgeOnboarding() {
  ...
  useEffect(() => {
    // Start opening doors immediately with a slightly snappier spring
    doorProgress.value = withSpring(1, {
      damping: 12,
      stiffness: 60,
      mass: 1.0,
    });

    // Viewfinder and scanner elements fade in while doors are opening (delay of 250ms)
    viewfinderOpacity.value = withDelay(250, withTiming(1, { duration: 400 }));
    viewfinderScale.value = withDelay(250, withTiming(1, { 
      duration: 500, 
      easing: Easing.out(Easing.back(1.2)) 
    }));

    // Laser appears and starts looping (delay of 400ms)
    laserOpacity.value = withDelay(400, withTiming(1, { duration: 250 }));
    laserPos.value = withDelay(400, withRepeat(
      withTiming(1, { duration: 1200, easing: Easing.inOut(Easing.ease) }),
      -1,
      true
    ));

    // Scanning status badge fades in (delay of 400ms)
    badgeScanningOpacity.value = withDelay(400, withTiming(1, { duration: 300 }));

    // Tomato detection badge (at 700ms delay) + haptic light tap
    badgeTomatoOpacity.value = withDelay(700, withTiming(1, { duration: 300 }, (ok) => {
      if (ok) runOnJS(haptic.light)();
    }));

    // Eggs detection badge (at 1100ms delay) + haptic light tap
    badgeEggOpacity.value = withDelay(1100, withTiming(1, { duration: 300 }, (ok) => {
      if (ok) runOnJS(haptic.light)();
    }));

    // Cheese detection badge (at 1500ms delay) + haptic light tap
    badgeCheeseOpacity.value = withDelay(1500, withTiming(1, { duration: 300 }, (ok) => {
      if (ok) runOnJS(haptic.light)();
    }));

    // Counters start counting up concurrently (delay of 400ms, duration of 1400ms)
  ...
  const bgStyle = useAnimatedStyle(() => {
    // Zoom in background image continuously from 0.85 to 1.22
    const scale = interpolate(doorProgress.value, [0, 1], [0.85, 1.22]);
    return { transform: [{ scale }] };
  });

  const overlayStyle = useAnimatedStyle(() => {
    // Start dark (0.7 opacity) and fade to 0.25 as doors open to simulate light coming on
    const opacity = interpolate(doorProgress.value, [0, 1], [0.7, 0.25], Extrapolation.CLAMP);
    return { opacity };
  });

  /* --- Fridge Door Animations --- */
  const doorsContainerStyle = useAnimatedStyle(() => {
    // Doors scale continuously from 0.85 to 1.22
    const scale = interpolate(doorProgress.value, [0, 1], [0.85, 1.22]);
    return { transform: [{ scale }] };
  });
        ...
        <View style={styles.fridgeContainer}>
          ...
          <View style={StyleSheet.absoluteFill}>
            <Animated.View style={[StyleSheet.absoluteFill, bgStyle]}>
              <Image source={require('../../assets/images/onboard_fridge.png')} style={styles.bgImage} />
            </Animated.View>
            <Animated.View style={[styles.overlay, overlayStyle]} />
          </View>

          {/* Cinematic Closed Fridge Doors (Zoom and slide open) */}
          <Animated.View style={[StyleSheet.absoluteFill, styles.doorsContainer, doorsContainerStyle]} pointerEvents="none">
            {/* Left Door */}
            ...
            <View style={styles.contentLayer}>
              ...
              <Animated.View style={[styles.hudRight, hudRightStyle]}>
                <Animated.View style={[styles.statusBadge, topBadgeStyle]}>
                  ...
                  <View style={{ overflow: 'hidden', height: 18, justifyContent: 'center', minWidth: 150 }}>
                    <Animated.Text style={[styles.statusText, scanningTextStyle]}>
                      {t('onboarding.scanning')}
                    </Animated.Text>
                    <Animated.Text style={[styles.statusText, completedTextStyle, { position: 'absolute' }]}>
                      {t('onboarding.scanCompleted')}
                    </Animated.Text>
                  </View>
                </Animated.View>
                
                <Animated.View style={[styles.statusBadge, { marginTop: 10, borderColor: colors.green + '88' }, tomatoBadgeStyle]}>
                  <Text style={[styles.statusText, { color: colors.green }]}>{t('onboarding.fridge.tomatoes')}</Text>
                </Animated.View>
                
                <Animated.View style={[styles.statusBadge, { marginTop: 10, borderColor: colors.orange + '88' }, eggBadgeStyle]}>
                  <Text style={[styles.statusText, { color: colors.orange }]}>{t('onboarding.fridge.eggs')}</Text>
                </Animated.View>
                
                <Animated.View style={[styles.statusBadge, { marginTop: 10, borderColor: colors.gold + '88' }, cheeseBadgeStyle]}>
                  <Text style={[styles.statusText, { color: colors.gold }]}>{t('onboarding.fridge.cheese')}</Text>
                </Animated.View>
              </Animated.View>
            </View>
          </Animated.View>
