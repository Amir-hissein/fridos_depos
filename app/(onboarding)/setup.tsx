...
const ACTIVITIES: { key: Activity; label: string; desc: string; icon: IconName }[] = [
  {
    key: 'sedentary',
    label: 'setup.activities.sedentary.label',
    desc: 'setup.activities.sedentary.desc',
    icon: 'monitor',
  },
  {
    key: 'active',
    label: 'setup.activities.active.label',
    desc: 'setup.activities.active.desc',
    icon: 'arm-flex',
  },
  {
    key: 'cardio',
    label: 'setup.activities.cardio.label',
    desc: 'setup.activities.cardio.desc',
    icon: 'heart-pulse',
  },
];
            ...
            return (
              <View key={i} style={ruler.slot}>
                <View
                  style={[
                    ruler.tick,
                    { height: major ? 26 : 14, backgroundColor: tint, opacity: major ? 0.9 : 0.35 },
                  ]}
                />
              </View>
            );
        ...
        {Array.from({ length: count }).map((_, i) => {
          ...
          return (
            <View key={i} style={wheel.item}>
              <Text style={[wheel.text, active && wheel.textActive]}>
      ...
      {PACES_DATA.map((p) => {
        ...
        return (
          ...
          >
            ...
            <Text style={[styles.paceCardRate, active && styles.paceCardRateActive]}>{p.rate}</Text>
          </PressableScale>
        );
      })}
    </View>
...
/** One checklist line that stays in the flow (stable layout) but only reveals —
 *  fading + rising in with a popping check — once the bar reaches its threshold. */
function LoadingCheckRow({ label, visible }: { label: string; visible: boolean }) {
  const { colors } = useTheme();
  const styles = useThemedStyles(makeStyles);
  ...
  const badgeScale = anim.interpolate({ inputRange: [0, 1], outputRange: [0.5, 1] });

  return (
    <Animated.View style={[styles.loadingCheckRow, { opacity: anim, transform: [{ translateY }] }]}>
      <Animated.View
        style={[styles.checkBadge, styles.checkBadgeDone, { transform: [{ scale: badgeScale }] }]}
      >
        <MaterialCommunityIcons name="check" size={14} color={colors.white} />
      </Animated.View>
      <Text style={[styles.loadingCheckTxt, styles.loadingCheckTxtDone]}>{label}</Text>
    </Animated.View>
  );
}
    ...
    <View style={styles.loadingWrap}>
      ...
      <View style={styles.loadingChecks}>
        {LOADING_CHECKS.map((ch) => (
          <LoadingCheckRow key={ch.t} label={t(ch.t)} visible={pct >= ch.at} />
        ))}
      </View>
    </View>
  );
...
export default function SetupScreen() {
  ...
  const wRange =
    weightUnit === 'lb' ? { min: 77, max: 551, step: 1 } : { min: 35, max: 250, step: 0.5 };
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={[styles.sexLabel, active && styles.sexLabelActive]}>
                      {sx === 'male' ? t('setup.step0.male') : t('setup.step0.female')}
                    </Text>
                  </PressableScale>
                );
              })}
        ...
        {step === 1 && (
          <>
            ...
            <View style={styles.weightCards}>
              <View style={styles.weightCol}>
                ...
                <View style={[styles.weightCard, styles.weightCardActive]}>
                  <Text style={[styles.weightVal, { color: colors.green }]}>
                    {toDisp(form.weight).toFixed(1)}
                  </Text>
              ...
              />
              <View style={styles.weightCol}>
                <Text style={[styles.weightCardLabel, { color: colors.goldDark }]}>
                  {t('setup.step1.target')}
                </Text>
                <View style={styles.weightCard}>
                  <Text style={[styles.weightVal, { color: colors.gold }]}>
                    {toDisp(form.targetWeight).toFixed(1)}
                  </Text>
                  <Text style={styles.weightUnit}>{weightUnit}</Text>
                </View>
              </View>
            ...
            <Ruler
              key={`tgt-${weightUnit}`}
              value={toDisp(form.targetWeight)}
              min={wRange.min}
              max={wRange.max}
              step={wRange.step}
              tint={colors.gold}
        ...
        {step === 3 && (
          <>
            ...
            <View style={{ alignSelf: 'center', marginTop: 8, marginBottom: 22 }}>
              <Segmented
                options={[
                  { key: 'gun', label: t('setup.step3.day') },
                  { key: 'hafta', label: t('setup.step3.week') },
                ]}
                value={paceUnit}
            ...
            <View style={styles.durationCard}>
              <Text style={styles.durationLabel}>
                {t('profile.weightHistoryPage.duration', {
                  defaultValue: 'DURÉE ESTIMÉE',
                }).toUpperCase()}
              </Text>
              <View style={styles.durationValueRow}>
                ...
                <Text style={styles.durationUnit}>
                  {paceUnit === 'hafta'
                    ? t('setup.step3.week', { defaultValue: 'SEMAINE' }).toLowerCase() +
                      (goalNumber > 1 ? 's' : '')
                    : t('setup.step3.day', { defaultValue: 'JOUR' }).toLowerCase() +
                      (goalNumber > 1 ? 's' : '')}
                </Text>
              </View>
            </View>
        ...
        {step === 7 &&
          (() => {
            ...
            const macroPills = [
              {
                label: t('setup.result.protein'),
                value: `${targets.protein}g`,
                color: colors.protein,
              },
              { label: t('setup.result.carbs'), value: `${targets.carbs}g`, color: colors.carbs },
              { label: t('setup.result.fat'), value: `${targets.fat}g`, color: colors.fat },
            ];
            return (
              <>
                ...
                {/* Carte héro — objectif calorique quotidien + date d'objectif */}
                <View style={styles.resHero}>
                  <Text style={styles.resHeroLabel}>{t('setup.result.calorieGoal')}</Text>
                  <View style={styles.resHeroRow}>
                    <MaterialCommunityIcons name="fire" size={26} color={colors.orange} />
                    <Text style={styles.resHeroValue}>{group(targets.kcal)}</Text>
                    <Text style={styles.resHeroUnit}>{t('setup.result.kcalLabel')}</Text>
                  </View>
                  <View style={[styles.resDatePill, styles.resHeroDate]}>
                    <MaterialCommunityIcons name="flag-checkered" size={15} color={colors.green} />
                  ...
                  <View style={styles.resPillsRow}>
                    {macroPills.map((p) => (
                      <View key={p.label} style={styles.resPill}>
                        <View style={[styles.resPillDot, { backgroundColor: p.color }]} />
                ...
                <View style={styles.resCard}>
                  ...
                  <View style={styles.resGaugeTrack}>
                    ...
                    />
                    <View
                      style={[styles.resGaugeThumb, { left: `${gaugePos * 100}%` as `${number}%` }]}
                    />
                  </View>
                ...
                <View style={styles.resTrackRow}>
                  <View style={styles.resTrackCard}>
                    <Text style={[styles.resTrackTitle, { color: colors.blue }]}>
                      {t('setup.result.waterTitle')}
                    </Text>
                    <MaterialCommunityIcons
                      name="cup-water"
                      size={34}
                      color={colors.blue}
                      style={styles.resTrackIcon}
                    />
                    <Text style={styles.resTrackText}>
                      {t('setup.result.waterText', { ml: group(targets.waterMl) })}
                    </Text>
                  </View>
                  <View style={styles.resTrackCard}>
                    <Text style={[styles.resTrackTitle, { color: colors.green }]}>
                      {t('setup.result.stepsTitle')}
                    </Text>
                    <MaterialCommunityIcons
                      name="walk"
                      size={34}
                    ...
                    />
                    <Text style={styles.resTrackText}>
                      {t('setup.result.stepsText', { steps: group(form.dailySteps) })}
                    </Text>
                  </View>
                </View>
              </>
...
const makeSegStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    wrap: {
      flexDirection: 'row',
...
const makeWheelStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    highlight: {
      position: 'absolute',
      left: 0,
      right: 0,
      top: ITEM_H * ((WHEEL_VISIBLE - 1) / 2),
      height: ITEM_H,
      borderRadius: 18,
      backgroundColor: colors.surface,
    },
...
const makeStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    ...
    resHeroRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 14 },
    resHeroValue: {
      fontFamily: 'Poppins_700Bold',
      fontSize: 46,
      color: colors.textPrimary,
    ...
    resPillDot: { width: 8, height: 8, borderRadius: 4, marginBottom: 1 },
