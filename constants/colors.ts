...
export const darkColors = {
  ...
  // Elevation (see `elevation()` in constants/layout). `shadowBase` is opaque;
  // strength is applied via shadowOpacity so the same tokens read correctly on
  // iOS. `isDark` lets the helper pick per-theme opacity.
  isDark: true,
  shadowBase: '#000000',
};
...
export const lightColors: typeof darkColors = {
  ...
  isDark: false,
  // Cool near-black tuned to the app's teal background — softer than pure black.
  shadowBase: '#0B1F1C',
};
